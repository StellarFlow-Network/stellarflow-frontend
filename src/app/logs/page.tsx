...
function mergeDecodedLogs(
  ...
  return logs.map((log) => {
    ...
    return decodedData ? { ...log, decodedData } : log;
  });
}
    ...
    <div className="min-h-screen bg-[#0a0a0a] p-8 text-gray-100" aria-busy="true">
      ...
      <div className="overflow-hidden rounded-xl border border-gray-800 bg-[#161b22]">
        ...
        {Array.from({ length: 6 }).map((_, index) => (
          <div
            key={index}
            className="grid grid-cols-[140px_120px_100px_1fr_150px_120px] items-center border-b border-gray-800/50"
          >
...
export default function LogsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const cachedLogs = await readIndexedLogs();
      if (!cancelled && cachedLogs && cachedLogs.length > 0) {
        setLogs(cachedLogs);
      }
      if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const xdrItems = logs
      ...
      .map((log) => ({
        id: log.id,
        xdr: log.message.replace("XDR: ", ""),
      }));

    if (xdrItems.length === 0) return;

    void batchDecode(`initial-${xdrItems.length}`, xdrItems)
      .then((results) => {
      ...
      .catch(() => {
        // Ignore worker decode failures here; the page still renders raw logs.
      });
  ...
  const filteredResults = useMemo<FilteredLogResult[]>(() => {
    return logs
      ...
      .filter((log) => includesQuery(log, searchQuery))
      .map((log) => ({
        item: log,
        matches: buildMatches(log, searchQuery),
      }));
    ...
    <div className="min-h-screen bg-[#0a0a0a] p-8 text-gray-100">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-3">
          {xdrDecoding && (
            <div className="flex animate-pulse items-center gap-2 rounded-full border border-purple-500/30 bg-purple-500/10 px-3 py-1.5">
          ...
          <button
            type="button"
            onClick={exportCsv}
            className="flex items-center gap-2 rounded-lg border border-gray-700 bg-[#161b22] px-4 py-2 text-sm text-gray-300"
          >
      ...
      <div className="mb-6 flex flex-col items-center gap-4 rounded-xl border border-gray-800 bg-[#161b22] p-4 md:flex-row">
        <div className="relative w-full flex-1">
          ...
          <input
            ...
            className="w-full rounded-md border border-gray-700 bg-[#0d1117] py-2 pl-10 pr-4 text-sm focus:border-blue-500 focus:outline-none"
          />
        </div>
        ...
        <div className="flex w-full items-center gap-2 md:w-auto">
          ...
          <select
            className="rounded-md border border-gray-700 bg-[#0d1117] px-4 py-2 text-sm focus:outline-none"
            value={filter}
            onChange={(event) =>
              setFilter(event.target.value as "all" | LogEntry["severity"])
            }
          >
      ...
      <div className="relative mb-6 overflow-hidden rounded-xl border border-gray-800 bg-[#161b22]">
        {!isOnline && (
          <div className="absolute inset-x-0 top-0 z-20 flex items-center justify-center gap-2 bg-yellow-500/90 px-4 py-1.5 text-xs font-bold text-black">
              ...
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                return (
                  <div
                    ...
                    className="grid grid-cols-[140px_120px_100px_1fr_150px_120px] items-center border-b border-gray-800/50 font-mono text-[13px]"
                  >
                    <div className="whitespace-nowrap px-6 py-4 text-gray-400">
                      {log.timestamp}
                    </div>
                    ...
                    <div className="px-6 py-4 text-right">
                      {log.txHash ? (
                        <span className="inline-flex items-center justify-end gap-1 text-blue-500">
                          <SearchHighlight
                            text={log.txHash}
        ...
        <div className="flex items-center justify-between border-t border-gray-800 p-4 text-sm text-gray-500">
          <span>
            Showing {filteredResults.length} of {logs.length} entries
          </span>
          <span className="flex items-center gap-2">
            <Icon id={ICON_IDS.wifi} size={14} />
            {isOnline ? "Live connection available" : "Offline"}
          </span>
        </div>
      </div>
    </div>
...
function SearchHighlight({
  ...
  const parts = useMemo(
    () => buildHighlightedParts(text, matches as [number, number][] | undefined),
    [text, matches],
  );
...
function SeverityIndicator({ severity }: { severity: LogEntry["severity"] }) {
  ...
  return (
    <div
      className={`flex w-fit items-center gap-1.5 rounded-full border border-transparent px-2 py-0.5 ${styles[severity]}`}
    >
      <div
